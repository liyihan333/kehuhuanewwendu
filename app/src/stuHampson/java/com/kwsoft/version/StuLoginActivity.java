    ...
    private void initPermission() {
        PermissionGen.with(StuLoginActivity.this)
                ...
                .permissions(
                        ...
                        Manifest.permission.READ_PHONE_STATE//获取电话权限
                )
                .request();
    }
...
//        Toast.makeText(this, "权限成功", Toast.LENGTH_SHORT).show();
        ...
        StuPra.phoneImei = TelephonyMgr.getDeviceId();
    }
...
//            proDia.show();
            if (StuPra.phoneImei != null && StuPra.phoneImei.length() > 0) {
                dialog.show();
                ...
                if (!nameValue.equals("") && !pwdValue.equals("")) {//判断用户名密码非空
                    ...
                    map.put(StuPra.phoneId, StuPra.phoneImei);
                    Log.e(TAG, "postLogin1: phoneId//"+StuPra.phoneImei);
                    Log.e(TAG, "postLogin1: map"+map.toString());
                    //请求
                    OkHttpUtils
                            .post()
                            .params(map)
                            ...
                            .execute(new EdusStringCallback(StuLoginActivity.this) {
                                ...
                                public void onResponse(String response, int id) {
                                    Log.e(TAG, "onResponse: postLogin1:" + response);
                                    check(response);
                                }
                            });
                } else {
                ...
                Toast.makeText(this, "没有获取相关权限，无法登陆！", Toast.LENGTH_SHORT).show();
            }
        }
    }
...
//            if (loginError.getError() != 0) {
            Map<String, Object> errorMap = JSON.parseObject(menuData,
                    new TypeReference<Map<String, Object>>() {
                    });
            if (!(String.valueOf(errorMap.get("error"))).equals("0")){
                Toast.makeText(this, String.valueOf(errorMap.get("errorMsg")), Toast.LENGTH_SHORT).show();
                dialog.dismiss();
            } else {
                //当成功登陆后存储正确的用户名和密码,
